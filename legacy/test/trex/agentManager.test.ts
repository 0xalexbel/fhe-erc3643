  ...
  describe('.callMint', () => {
    describe('when specified identity is missing the SupplyModifier role', () => {
      it('Should revert', async () => {
        ...
        await expect(
          agentManager.connect(aliceWallet).callMint(bobWallet.address, 1000, aliceIdentity),
    ...
    describe('when identity has the SupplyModifier role and the sender is authorized for it', () => {
      it('Should perform the mint', async () => {
        ...
        const mintTx = await agentManager.connect(aliceWallet).callMint(bobWallet.address, 1000, aliceIdentity);
  ...
  describe('.callBurn', () => {
    ...
    describe('when identity has the SupplyModifier role and the sender is authorized for it', () => {
      it('Should perform the burn', async () => {
        ...
        const burnTx = await agentManager.connect(bobWallet).callBurn(bobWallet.address, 200, bobIdentity);

        await expect(burnTx).to.emit(token, 'Transfer').withArgs(bobWallet.address, hre.ethers.ZeroAddress, 200);
      });
    });
  });
    ...
    describe('when identity has the Freezer role and the sender is authorized for it', () => {
      it('Should perform the freeze of partial tokens', async () => {
        ...
        const freezeTx = await agentManager
          .connect(aliceWallet)
          .callFreezePartialTokens(aliceWallet.address, 100, aliceIdentity);
  ...
  describe('.callUnfreezePartialTokens', () => {
    ...
    describe('when identity has the Freezer role and the sender is authorized for it', () => {
      it('Should perform the unfreeze of partial tokens', async () => {
        ...
        const freezeTx = await agentManager
          .connect(aliceWallet)
          .callUnfreezePartialTokens(aliceWallet.address, 100, aliceIdentity);
