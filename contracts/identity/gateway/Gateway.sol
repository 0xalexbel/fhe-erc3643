...
contract Gateway is Ownable {
    ...
    constructor(address idFactoryAddress, address[] memory signersToApprove) Ownable(msg.sender) {
        ...
        for (uint256 i = 0; i < signersToApprove.length; i++) {
            _approvedSigners[signersToApprove[i]] = true;
        }

        _idFactory = IdFactory(idFactoryAddress);
    }
    ...
    function revokeSigner(address signer) external onlyOwner {
        ...
        delete _approvedSigners[signer];
    ...
    function deployIdentityWithSalt(
        ...
        if (_revokedSignatures[signature]) {
            revert RevokedSignature(signature);
        }

        return _idFactory.createIdentity(identityOwner, salt);
    }
    ...
    function deployIdentityWithSaltAndManagementKeys(
        ...
        return _idFactory.createIdentityWithManagementKeys(identityOwner, salt, managementKeys);
    }
    ...
    function deployIdentityForWallet(address identityOwner) external returns (address) {
        ...
        return _idFactory.createIdentity(identityOwner, Strings.toHexString(identityOwner));
    }
    ...
    function revokeSignature(bytes calldata signature) external onlyOwner {
        ...
        _revokedSignatures[signature] = true;
    ...
    function approveSignature(bytes calldata signature) external onlyOwner {
        ...
        delete _revokedSignatures[signature];
    ...
    function transferFactoryOwnership(address newOwner) external onlyOwner {
        _idFactory.transferOwnership(newOwner);
    }
    ...
    function callFactory(bytes memory data) external onlyOwner {
        (bool success, ) = address(_idFactory).call(data);
