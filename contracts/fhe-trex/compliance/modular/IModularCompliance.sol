...
import {einput, euint64, ebool} from "fhevm/lib/TFHE.sol";
...
interface IModularCompliance {
     ...
     *  @param _eamount The amount of tokens involved in the transfer
     *  This function calls moduleTransferAction() on each module bound to the compliance contract
     */
    function transferred(address _from, address _to, euint64 _eamount) external;
    function transferred(address _from, address _to, einput encryptedAmount, bytes calldata inputProof) external;
     ...
     *  This function can be called ONLY by the token contract bound to the compliance
     *  @param _to The address of the receiver
     *  @param _eamount The amount of tokens involved in the minting
     *  This function calls moduleMintAction() on each module bound to the compliance contract
     */
    function created(address _to, euint64 _eamount) external;
    function created(address _to, einput encryptedAmount, bytes calldata inputProof) external;
     ...
     *  @param _eamount The amount of tokens involved in the burn
     *  This function calls moduleBurnAction() on each module bound to the compliance contract
     */
    function destroyed(address _from, euint64 _eamount) external;
    function destroyed(address _from, einput encryptedAmount, bytes calldata inputProof) external;
     ...
     */
    function canTransfer(address _from, address _to, euint64 _eamount) external returns (ebool);
