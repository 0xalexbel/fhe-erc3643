...
contract ConditionalTransferModule is AbstractModuleUpgradeable {
     ...
     */
    event TransferApproved(address _from, address _to, euint64 _eamount, address _token);
     ...
     *  `_token` is the token address of the token concerned by the approval
     */
    event ApprovalRemoved(address _from, address _to, euint64 _eamount, address _token);
     ...
     *  @param _eamounts the array of tokens amounts that `_from` would send to `_to`
    ...
    function batchApproveTransfers(
        ...
        for (uint256 i = 0; i < _from.length; i++) {
            approveTransfer(_from[i], _to[i], _eamounts[i]);
        }
    }
     ...
     *  @param _from the array of addresses of the transfer senders
     *  @param _to the array of addresses of the transfer receivers
     *  @param _eamounts the array of token amounts that `_from` were allowed to send to `_to`
     *  Only a bound compliance can call this function
    ...
    function batchUnApproveTransfers(
        ...
        for (uint256 i = 0; i < _from.length; i++) {
            unApproveTransfer(_from[i], _to[i], _eamounts[i]);
        }
    }
    ...
    function moduleTransferAction(address _from, address _to, euint64 _evalue) external override onlyComplianceCall {
        ...
        if (_transfersApproved[msg.sender][transferHash] > 0) {
            ...
            emit ApprovalRemoved(_from, _to, _evalue, IModularCompliance(msg.sender).getTokenBound());
        }
    }
    ...
    function moduleCheck(
        ...
        bool approved = isTransferApproved(_compliance, transferHash);
        ebool eapproved = TFHE.asEbool(approved);

        TFHE.allowTransient(eapproved, msg.sender);
        return eapproved;
    }
     ...
     *  @param _eamount the amount of tokens that `_from` would send to `_to`
     *  Only a bound compliance can call this function
     *  emits a `TransferApproved` event
     */
    function approveTransfer(address _from, address _to, euint64 _eamount) public onlyComplianceCall {
        // No TFHE permission needed
        bytes32 transferHash = calculateTransferHash(
            _from,
            _to,
            _eamount,
        ...
        _transfersApproved[msg.sender][transferHash]++;
        emit TransferApproved(_from, _to, _eamount, IModularCompliance(msg.sender).getTokenBound());
    }
     ...
     *  @param _from the address of the transfer sender
     *  @param _to the address of the transfer receiver
     *  @param _eamount the amount of tokens that `_from` was allowed to send to `_to`
     *  Only a bound compliance can call this function
     *  emits an `ApprovalRemoved` event
     */
    function unApproveTransfer(address _from, address _to, euint64 _eamount) public onlyComplianceCall {
        // No TFHE permission needed
        bytes32 transferHash = calculateTransferHash(
            _from,
            _to,
        ...
        _transfersApproved[msg.sender][transferHash]--;
        emit ApprovalRemoved(_from, _to, _eamount, IModularCompliance(msg.sender).getTokenBound());
    }
    ...
    function calculateTransferHash(
        ...
        bytes32 transferHash = keccak256(abi.encode(_from, _to, _eamount, _token));
        return transferHash;
    }
