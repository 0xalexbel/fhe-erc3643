...
async function deployAgentFixture() {
  ...
  const agentRole = await hre.ethers.deployContract('AgentRole');
    ...
    describe('when the sender is the owner', () => {
      describe('when address to add is the zero address', () => {
        it('should reverts', async () => {
          ...
          await expect(agentRole.connect(ownerWallet).addAgent(hre.ethers.ZeroAddress)).to.be.revertedWith(
    ...
    describe('when the sender is the owner', () => {
      describe('when address to add is the zero address', () => {
        it('should reverts', async () => {
          ...
          await expect(agentRole.connect(ownerWallet).removeAgent(hre.ethers.ZeroAddress)).to.be.revertedWith(
            'invalid argument - zero address',
          );
        });
      });
