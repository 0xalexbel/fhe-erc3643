...
describe('Token - Information', () => {
  describe('.setName()', () => {
    describe('when the caller is not the owner', () => {
      it('should revert', async () => {
        ...
        await expect(token.connect(anotherWallet).setName('My Token')).to.be.revertedWithCustomError(
          token,
  ...
  describe('.setSymbol()', () => {
    describe('when the caller is not the owner', () => {
      it('should revert', async () => {
        ...
        await expect(token.connect(anotherWallet).setSymbol('UpdtTK')).to.be.revertedWithCustomError(
          token,
          'OwnableUnauthorizedAccount',
        );
      });
  ...
  describe('.compliance()', () => {
    it('should return the compliance address', async () => {
      ...
      expect(await token.compliance()).to.equal(compliance);
    });
  });
  ...
  describe('.freezePartialTokens', () => {
    describe('when sender is not an agent', () => {
      it('should revert', async () => {
        ...
        await expect(
          token.connect(anotherWallet)['freezePartialTokens(address,uint256)'](anotherWallet.address, 1),
  ...
  describe('.unfreezePartialTokens', () => {
    describe('when sender is not an agent', () => {
      it('should revert', async () => {
        ...
        await expect(
          token.connect(anotherWallet)['unfreezePartialTokens(address,uint256)'](anotherWallet.address, 1),
        ).to.be.revertedWith('AgentRole: caller does not have the Agent role');
      });
    });
