  ...
  describe('.addTrustedIssuer()', () => {
    describe('when sender is not the owner', () => {
      it('should revert', async () => {
        ...
        await expect(
          trustedIssuersRegistry.connect(anotherWallet).addTrustedIssuer(anotherWallet.address, [10]),
    ...
    describe('when sender is the owner', () => {
      describe('when issuer to add is zero address', () => {
        it('should revert', async () => {
          ...
          await expect(
            trustedIssuersRegistry.connect(deployer).addTrustedIssuer(ethers.ZeroAddress, [10]),
      ...
      describe('when claim topics array is empty', () => {
        it('should revert', async () => {
          ...
          await expect(
            trustedIssuersRegistry.connect(deployer).addTrustedIssuer(deployer.address, []),
      ...
      describe('when claim topics array exceeds 15 topics', () => {
        it('should revert', async () => {
          ...
          await expect(
            trustedIssuersRegistry.connect(deployer).addTrustedIssuer(deployer.address, claimTopics),
  ...
  describe('.removeTrustedIssuer()', () => {
    describe('when sender is not the owner', () => {
      it('should revert', async () => {
        ...
        await expect(
          trustedIssuersRegistry.connect(anotherWallet).removeTrustedIssuer(anotherWallet.address),
        ).to.be.revertedWithCustomError(trustedIssuersRegistry, 'OwnableUnauthorizedAccount');
      });
    });
    ...
    describe('when sender is the owner', () => {
      describe('when issuer to remove is zero address', () => {
        it('should revert', async () => {
          ...
          await expect(
            trustedIssuersRegistry.connect(deployer).removeTrustedIssuer(ethers.ZeroAddress),
          ).to.be.revertedWith('invalid argument - zero address');
        });
      });
      ...
      describe('when issuer is not registered', () => {
        it('should revert', async () => {
          ...
          await expect(
            trustedIssuersRegistry.connect(deployer).removeTrustedIssuer(deployer.address),
    ...
    describe('when sender is the owner', () => {
      describe('when issuer to update is zero address', () => {
        it('should revert', async () => {
          ...
          await expect(
            trustedIssuersRegistry.connect(deployer).updateIssuerClaimTopics(ethers.ZeroAddress, [10]),
          ).to.be.revertedWith('invalid argument - zero address');
        });
      });
      ...
      describe('when issuer is not registered', () => {
        it('should revert', async () => {
          ...
          await expect(
            trustedIssuersRegistry.connect(deployer).updateIssuerClaimTopics(deployer.address, [10]),
          ).to.be.revertedWith('NOT a trusted issuer');
        });
      });
      ...
      describe('when claim topics array have more that 15 elements', () => {
        it('should revert', async () => {
          ...
          await expect(
            trustedIssuersRegistry.connect(deployer).updateIssuerClaimTopics(claimIssuerContract, claimTopics),
          ).to.be.revertedWith('cannot have more than 15 claim topics');
        });
      });
      ...
      describe('when claim topics array is empty', () => {
        it('should revert', async () => {
          ...
          await expect(
            trustedIssuersRegistry.connect(deployer).updateIssuerClaimTopics(claimIssuerContract, []),
      ...
      describe('when issuer is registered', () => {
        it('should update the topics of the trusted issuers', async () => {
          ...
          const tx = await trustedIssuersRegistry
            .connect(deployer)
            .updateIssuerClaimTopics(claimIssuerContract, [66, 100]);
          await expect(tx)
            .to.emit(trustedIssuersRegistry, 'ClaimTopicsUpdated')
            .withArgs(claimIssuerContract, [66, 100]);

          await expect(trustedIssuersRegistry.hasClaimTopic(claimIssuerContract, 66)).to.eventually.be.true;
          await expect(trustedIssuersRegistry.hasClaimTopic(claimIssuerContract, 100)).to.eventually.be.true;
          await expect(trustedIssuersRegistry.hasClaimTopic(claimIssuerContract, claimTopics[0])).to.eventually.be
            .false;
          await expect(trustedIssuersRegistry.getTrustedIssuerClaimTopics(claimIssuerContract)).to.eventually.deep.eq([
            66, 100,
          ]);
        });
      });
  ...
  describe('.getTrustedIssuerClaimTopics()', () => {
    describe('when issuer is not registered', () => {
      it('should revert', async () => {
        ...
        await expect(
          trustedIssuersRegistry.connect(deployer).getTrustedIssuerClaimTopics(deployer.address),
