  ...
  describe('.setFactory()', () => {
    describe('when called by not owner', () => {
      it('should revert', async () => {
        ...
        const gateway = await hre.ethers.deployContract(
          'TREXGateway',
          [hre.ethers.ZeroAddress, false],
          context.accounts.deployer,
        );
        await context.factories.trexFactory.transferOwnership(gateway);

        await expect(
          gateway.connect(context.accounts.anotherWallet).setFactory(context.factories.trexFactory),
    ...
    describe('when called by owner', () => {
      describe('if called with zero address', () => {
        it('should revert', async () => {
          ...
          await expect(gateway.setFactory(hre.ethers.ZeroAddress)).to.be.revertedWithCustomError(
            gateway,
            'ZeroAddress',
          );
        });
  ...
  describe('.setPublicDeploymentStatus()', () => {
    describe('when called by not owner', () => {
      it('should revert', async () => {
        ...
        await expect(
          gateway.connect(context.accounts.anotherWallet).setPublicDeploymentStatus(true),
        ).to.be.revertedWithCustomError(gateway, 'OwnableUnauthorizedAccount');
      });
    });
    describe('when called by owner', () => {
  ...
  describe('.transferFactoryOwnership()', () => {
    describe('when called by not owner', () => {
      it('should revert', async () => {
        ...
        const gateway = await hre.ethers.deployContract(
          'TREXGateway',
          [context.factories.trexFactory, false],
          context.accounts.deployer,
        );
        await context.factories.trexFactory.transferOwnership(gateway);
  ...
  describe('.enableDeploymentFee()', () => {
    describe('when called by not owner', () => {
      it('should revert', async () => {
        ...
        await expect(
          gateway.connect(context.accounts.anotherWallet).enableDeploymentFee(true),
        ).to.be.revertedWithCustomError(gateway, 'OwnableUnauthorizedAccount');
      });
    });
    describe('when called by owner', () => {
    ...
    describe('when called by not owner', () => {
      it('should revert', async () => {
        ...
        await expect(
          gateway
            .connect(context.accounts.anotherWallet)
            .setDeploymentFee(100, context.suite.token, context.accounts.anotherWallet.address),
        ).to.be.revertedWithCustomError(gateway, 'OwnableUnauthorizedAccount');
      });
    });
    describe('when called by owner', () => {
      describe('if required parameters are not filled', () => {
        it('should revert', async () => {
          ...
          await expect(
            gateway.setDeploymentFee(100, hre.ethers.ZeroAddress, context.accounts.deployer.address),
          ).to.be.revertedWithCustomError(gateway, 'ZeroAddress');
          await expect(
            gateway.setDeploymentFee(100, context.suite.token, hre.ethers.ZeroAddress),
          ).to.be.revertedWithCustomError(gateway, 'ZeroAddress');
        });
      });
    ...
    describe('when called by owner', () => {
      describe('if deployer already exists', () => {
        it('should revert', async () => {
          ...
          await expect(gateway.addDeployer(context.accounts.tokenAgent.address)).to.be.revertedWithCustomError(
            gateway,
    ...
    describe('when called by agent', () => {
      describe('if deployer already exists', () => {
        it('should revert', async () => {
          ...
          await gateway.addDeployer(context.accounts.tokenAgent.address);
          await expect(
            gateway.connect(context.accounts.tokenAgent).addDeployer(context.accounts.tokenAgent.address),
    ...
    describe('when called by owner', () => {
      describe('when adding a batch of deployers that includes an already registered deployer', () => {
        it('should revert', async () => {
          ...
          await gateway.addDeployer(context.accounts.tokenAgent.address);
          const newDeployers = Array.from({ length: 9 }, () => hre.ethers.Wallet.createRandom().address);
      ...
      describe('when adding a batch of more than 500 deployers', () => {
        it('should revert', async () => {
          ...
          const duplicateAddress = hre.ethers.Wallet.createRandom().address;
      ...
      describe('if new deployers', () => {
        ...
        it('should add 10 new deployers', async () => {
          ...
          await context.factories.trexFactory.transferOwnership(gateway);
          const newDeployers = Array.from({ length: 10 }, () => hre.ethers.Wallet.createRandom().address);
    ...
    describe('when called by agent', () => {
      describe('when adding a batch of deployers that includes an already registered deployer', () => {
        it('should revert', async () => {
          ...
          newDeployers.splice(randomIndex, 0, context.accounts.tokenAgent.address);
          await expect(
            gateway.connect(context.accounts.anotherWallet).batchAddDeployer(newDeployers),
          ).to.be.revertedWithCustomError(gateway, 'DeployerAlreadyExists');
        });
      });
      describe('if new deployers', () => {
    ...
    describe('when called by owner', () => {
      describe('if deployer does not exist', () => {
        it('should revert', async () => {
          ...
          await expect(gateway.removeDeployer(context.accounts.tokenAgent.address)).to.be.revertedWithCustomError(
            gateway,
        ...
        describe('when called by an agent', () => {
          describe('if at least one deployer does not exist', () => {
            it('should revert', async () => {
              ...
              const deployers = Array.from({ length: 9 }, () => hre.ethers.Wallet.createRandom().address);
              await gateway.batchAddDeployer(deployers);
              deployers.push(context.accounts.tokenAgent.address);
              await gateway.addAgent(context.accounts.tokenAgent.address);
              await expect(
                gateway.connect(context.accounts.tokenAgent).batchRemoveDeployer(deployers),
              ).to.be.revertedWithCustomError(gateway, 'DeployerDoesNotExist');
            });
          });
          ...
          describe('if all deployers exist', () => {
            it('should remove deployers', async () => {
              ...
              const deployers = Array.from({ length: 10 }, () => hre.ethers.Wallet.createRandom().address);
              await gateway.batchAddDeployer(deployers);
              await gateway.addAgent(context.accounts.tokenAgent.address);
    ...
    describe('when called by owner', () => {
      describe('if discount out of range', () => {
        it('should revert', async () => {
          ...
          await expect(
            gateway.applyFeeDiscount(context.accounts.anotherWallet.address, 12000),
    ...
    describe('when called by not owner', () => {
      it('should revert', async () => {
        ...
        await expect(
          gateway
            .connect(context.accounts.anotherWallet)
            .batchApplyFeeDiscount([context.accounts.anotherWallet.address], [5000]),
        ).to.be.revertedWithCustomError(gateway, 'OnlyAdminCall');
      });
    });
    describe('when called by owner', () => {
      describe('if discount out of range', () => {
        it('should revert', async () => {
          ...
          await expect(
            gateway.batchApplyFeeDiscount([context.accounts.anotherWallet.address], [12000]),
          ).to.be.revertedWithCustomError(gateway, 'DiscountOutOfRange');
        });
      });
    ...
    describe('when called by agent', () => {
      describe('if any discount in the batch is out of range', () => {
        it('should revert the whole batch', async () => {
          ...
          await gateway.addAgent(context.accounts.tokenAgent.address);
          await expect(
            gateway.connect(context.accounts.tokenAgent).batchApplyFeeDiscount(deployers, discounts),
          ).to.be.revertedWithCustomError(gateway, 'DiscountOutOfRange');
        });
      });
      describe('if all discounts are valid', () => {
        it('should apply discounts to all deployers', async () => {
          ...
          await gateway.setDeploymentFee(deploymentFee, context.suite.token, context.accounts.deployer.address);
      ...
      describe('when public deployments are enabled', () => {
        describe('when try to deploy on behalf', () => {
          it('should revert', async () => {
            ...
            const gateway = await hre.ethers.deployContract(
              'TREXGateway',
              [context.factories.trexFactory, true],
              context.accounts.deployer,
            );
            await context.factories.trexFactory.transferOwnership(gateway);
        ...
        describe('when deployment fees are activated', () => {
          describe('when caller has no discount', () => {
            it('should deploy a token for full fee', async () => {
              ...
              await context.factories.trexFactory.transferOwnership(gateway);
              const feeToken = await hre.ethers.deployContract('TestERC20', ['FeeToken', 'FT']);
              await feeToken.mint(context.accounts.anotherWallet.address, 100000);
              await gateway.setDeploymentFee(20000, feeToken, context.accounts.deployer.address);
              await gateway.enableDeploymentFee(true);

              await feeToken.connect(context.accounts.anotherWallet).approve(gateway, 20000);
              const tx = await gateway.connect(context.accounts.anotherWallet).deployTREXSuite(
                {
                  owner: context.accounts.anotherWallet.address,
                  name: 'Token name',
        ...
        describe('when deployment fees are activated', () => {
          describe('when caller has no discount', () => {
            it('should deploy tokens for full fee in a batch', async () => {
              ...
              await feeToken.connect(context.accounts.anotherWallet).approve(gateway, 100000);
          ...
          describe('when caller has 50% discount', () => {
            it('should deploy tokens for half fee in a batch', async () => {
              ...
              await gateway.enableDeploymentFee(true);
              await gateway.applyFeeDiscount(context.accounts.anotherWallet.address, 5000);
              await feeToken.connect(context.accounts.anotherWallet).approve(gateway, 50000);
